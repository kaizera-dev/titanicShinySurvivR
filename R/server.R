titanic_server <- function(input, output, session) {
  ...
  shiny::observeEvent(reactive_titanic(), {
    validation_error <- validate_titanic_data(reactive_titanic())
...
These edits do not affect prediction results."
  ...
  prepped_data <- shiny::reactive({
    preprocess_titanic_data(train_data = raw_train(), test_data = raw_test())
  })
  ...
  dec_tree <- shiny::reactive({
    ...
    rpart::rpart(Survived ~ .,
      data = prepped_train(), method = "class",
  ...
  rand_forest <- shiny::reactive({
    shiny::req(prepped_train())
    randomForest::randomForest(Survived ~ .,
      data = prepped_train(),
      ntree = 600,
  ...
  output$confus <- shiny::renderText({
    paste(
      ...
      "  1 = SURVIVED\n",
      "  0 = DID NOT SURVIVE\n\n",
      generate_confusion_matrix_summary(conf_mx()),
      collapse = ""
    )
  })
  ...
  output$pred <- shiny::renderText({
    ...
    paste(
      ...
      extract_important_features(model_obj, input$model),
      "\n"
    )
  })

  output$survival_plot <- shiny::renderPlot({
    plot_survival_metric(reactive_titanic(), input$metric)
  })

  output$probability_plot <- shiny::renderPlot({
    plot_predicted_probabilites(
  ...
  roc_auc_output <- shiny::reactive({
    plot_roc_auc(prepped_test(), model_list(), input$model)
  })
  ...
  output$intro <- renderUI({
    HTML("
<p>Thank you for taking the time to check out my first ever project, <b>Titanic SurvivR</b>! This Shiny app is based on the classic Kaggle Titanic competition—with a twist: you get to find out whether <em>you</em> would have survived the sinking.</p>
...
<ol>
  ...
  <li>Click on the <b>Prediction</b> tab to see your results.</li>
</ol>
...
<ul>
  <li><b>Introduction:</b> Overview of the app's purpose and how to use it.</li>
  <li><b>Prediction:</b> View your survival outcome based on selected input features and see which features matter the most.</li>
  <li><b>Survival Patterns:</b> Visualise how specific features relate to survival rates, and view the predicted probability distribution for each model.</li>
  <li><b>Confusion Matrix:</b> See how well each model classifies survival outcomes on test data.</li>
  <li><b>ROC-AUC:</b> Evaluate model discrimination using ROC curves and AUC values.</li>
  <li><b>Data Preview:</b> Examine the raw dataset used for model training and testing.</li>
  <li><b>Titanic Deck Layout:</b> Visual reference of the ship’s deck plan used in feature engineering.</li>
</ul>

<h3>💡 Why I Built This</h3>
<p>The goal of this project was to explore the <b>Shiny framework</b> and get hands-on with applied <b>machine learning and statistics</b> in R. It really has helped me learn a lot in both fields. Looking back to when I first started, if you had told me ROC-AUC was a Star Wars character, I would have believed you.</p>
